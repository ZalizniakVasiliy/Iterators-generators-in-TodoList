...
(function () {
  const todoList = {
    ...
    preFillHandler() {
      ...
      /* Solution with generator for output data from local storage */

      function* generatorItems(objectsArr) {
        for (const obj of objectsArr) {
          yield obj;
        }
      }

      for (const template of generatorItems(data)) {
        todoContainer.prepend(this.createTemplate(template));
      }
      /* --------------------------------------------------------- */
    },
